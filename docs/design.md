...
- GitHub 監視: 初期実装はポーリングのみ。Webhook は将来拡張
...
現行実装に Webhook 受信エンドポイントはなく、ここでの Webhook は将来拡張として扱う。
将来的には Webhook アダプタを追加可能にする。
...
- GitHub ポーリング自体の失敗はジョブ通知とは別系統の運用通知として扱う
- 例:
  - `gh auth token` 取得失敗
  - GitHub API / GraphQL の認可不足
  - `issue_project` 監視時の `read:project` 権限不足
- これらは対象ジョブを作らず、バックエンドログに加えて Web UI 上でも確認可能にする
- 同一原因のエラーを毎回大量通知しないよう、一定時間の集約または重複抑止を前提とする
...
- GitHub ポーリング設定 / 認可エラー: 対象ジョブは作らず、運用向け通知とログへ記録
