...
ジョブ一覧画面では、`Kind` と状態グループで絞り込んだ後、`取得日時` を基準に並び順を切り替えられるようにする。

- 既定の並び順は取得日時の新しい順
- 利用者は取得日時の古い順にも切り替えられる
- 並び順はフィルター後の一覧に対して適用する
- 自動更新や手動更新後も、選択中の並び順を維持する
- 一覧には `subStatus` を表示しない
- ジョブ詳細では状態チップの横に `subStatus` を表示する
- `subStatus` は実装ジョブの途中経過を表し、`実装(n回目)` と `検証(n回目)` を使う
- 実装系ジョブの詳細では、成果物とは別にソース差分を確認できるようにする
- 差分は「差分確認」ボタン押下時に取得し、差分ビューで表示する
- 差分ビューには「戻る」を置き、ジョブ詳細の通常表示へ戻れるようにする
- 承認後や完了後でも、対象 worktree が残っている限り差分を再表示できるようにする

一覧のソートや `subStatus` の表示はフロントエンド側で行い、API は一覧に必要なデータを返すだけにする。
...
実装系ジョブでは、同じく最終状態に到達したあともソース差分をジョブ詳細から確認できるようにする。
