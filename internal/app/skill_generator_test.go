...
`
	...
	request := domain.SkillGenerationRequest{TestCommand: "npm test"}
	if err := validateGeneratedSkill(dir, definition, request); err != nil {
		t.Fatalf("validateGeneratedSkill() error = %v", err)
	}
}
...
func TestSkillGenerationPromptRequiresUsableSkillSections(t *testing.T) {
	path := filepath.Join("..", "..", "prompt", "skill_generation_prompt.tmpl")
	prompt, err := renderSkillGenerationPrompt(path, skillGenerationPromptData{
		ProviderDisplayName: "Codex",
		StageDir:            t.TempDir(),
		ProjectContext:      "GoとVue.jsのプロジェクト",
		TestCommand:         "go test ./...",
		Missing: []skillPromptDefinition{{
			Name:        "implement-from-design",
			Purpose:     string(domain.SkillPurposeIssueImplementation),
			DisplayName: "Issue実装",
		}},
		IsCodex: true,
	})
	...
	for _, required := range []string{
		"## 処理内容",
		"## 必須出力形式",
		"入力として確認する情報、判断条件、実際に行う作業、検証方法、結果のまとめ方",
		"go test ./...",
		"agents/openai.yaml",
	} {
		if !strings.Contains(prompt, required) {
			t.Fatalf("rendered prompt is missing %q", required)
		}
	}
}
...
`,
	...
	for name, content := range files {
		content = strings.Replace(content, "## 必須出力形式", "## 処理内容\n対象情報を確認し、作業と検証を完了する。\n\n## 必須出力形式", 1)
