...
func startImplementationWorker(ctx context.Context, root string, cfg *config.Service, orch *orchestrator.Orchestrator, logger *log.Logger) error {
	...
	go func() {
		...
		for {
			if err := runPendingImplementations(ctx, cfg, orch, runner, testRunner, logger); err != nil && ctx.Err() == nil {
...
func runPendingImplementations(ctx context.Context, cfg *config.Service, orch *orchestrator.Orchestrator, runner *skill.Runner, testRunner *executor.TestRunner, logger *log.Logger) error {
