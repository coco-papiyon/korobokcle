...
func generateImprovementDraft(ctx context.Context, cfg *config.Service, orch *orchestrator.Orchestrator, jobID string, sourceEventType string, logger *log.Logger) (improvementGenerationResult, error) {
	...
	improvementWorkspaceDir := artifacts.RepositoryWorkerImprovementWorkspaceDir(cfg.Root(), cfg.App().ArtifactsDir, job.Repository, config.ResolveImprovementBranch(repositoryConfig))
	workFiles := repositoryImprovementWorkFiles(improvementWorkspaceDir, repositoryConfig.ImprovementDir, job.ID, job.Title)
	...
	currentResultMarkdown, err := loadCurrentImprovementResult(comment, workFiles)
	if err != nil {
		return improvementGenerationResult{}, err
	}
...
func generateImprovementDraftContent(
	...
	aiResult, err := generateImprovementDraftWithAI(ctx, cfg, job, source, phases, existingImprovements, inputMarkdown, currentResultMarkdown, workDir, artifactFiles, logger)
	if err == nil {
		if logger != nil {
...
func loadCurrentImprovementResult(comment string, workFiles improvementWorkFiles) (string, error) {
	if strings.TrimSpace(comment) == "" {
		return "", nil
	}
	for _, path := range []string{workFiles.DraftPath, workFiles.LegacyDraftPath} {
